var core = new Vue({
  ...
  data: {
    ...
    servicos: [
      ...
      {
        nome: "Taxi",
        url: "https://cdn.discordapp.com/attachments/895016171651334154/1047512126211764264/Copia_de_Metropolitano.png",
        nivel: 1,
        valorPago: 100,
        valorPagar: 0,
        valorRecebido: 0,
...
function comprarLoja() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch("http://monkey_onibus/buy", { method: "POST", body: JSON.stringify({
...
function sacarDinheiro() {
  return new Promise((resolve, reject) => {
    Swal.fire({
      ...
      if (res.isConfirmed) {
        fetch("http:/monkey_onibus/withdraw",{
          method:"POST",
          body:JSON.stringify({
            id: core.idLoja,
            valor: res.value
...
function close() {
  return new Promise(function (resolve, reject) {
    fetch("http://monkey_onibus/close", { method: "POST" })
      .then(function () {
...
function iniciarRota(nome) {
  fetch("http://monkey_onibus/initRoute", {
    method: "POST",
    body: JSON.stringify({
      nome: nome,
    }),
...
function aplicarPorcentagem(valor, id){
  return new Promise((resolve, reject)=>{
    Swal.fire({
      ...
      if (res.isConfirmed) {
        fetch("http:/monkey_onibus/setPorcentagem",{
          method:"POST",
          body:JSON.stringify({
            id: id,
            valor: valor
