...
local inPed = nil
local selecionado = 1
local CoordenadaX,CoordenadaY,CoordenadaZ = -1311.22, -395.68, 36.69
local quantidade = 0
...
local pagamento = {
	...
	["plastic"] = {
		[1] = { ["item"] = "legos" , ["qtd_min"] = 50 , ["qtd_max"] = 70 },
		[2] = { ["item"] = "ominitrix" , ["qtd_min"] = 50 , ["qtd_max"] = 70 }
	},
	["drogas"] = {
		[1] = { ["item"] = "farinha" , ["qtd_min"] = 0 , ["qtd_max"] = 2 },
		[2] = { ["item"] = "cannabisseed" , ["qtd_min"] = 0 , ["qtd_max"] = 2 },
		[3] = { ["item"] = "doril" , ["qtd_min"] = 0 , ["qtd_max"] = 2 },
		[4] = { ["item"] = "xarope" , ["qtd_min"] = 0 , ["qtd_max"] = 2 },
		[5] = { ["item"] = "anador" , ["qtd_min"] = 0 , ["qtd_max"] = 2 }
	},
	["mochila"] = {
...
Citizen.CreateThread(function()
	while true do
		...
		if not servico then
			...
			for k, value in pairs(locais_iniciar) do 
				local distance = #(coords - vector3(value.x,value.y,value.z))
				if distance <= 5 then
					time = 1
					DrawMarker(21,value.x,value.y,value.z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,255,0,0,50,0,0,0,1)
					if distance <= 1.2 then
						drawTxt("PRESSIONE  ~r~E~w~  PARA INICIAR AS COLETAS DE PEÇAS DE ARMAS",4,0.5,0.93,0.50,255,255,255,180)
						if IsControlJustPressed(0,38) then

							if value.permissao == "public" then

								if value.tipo == "drogas" then 
									tipo_rota = "drogas"
									locs = drogas
								end

								if value.tipo == "mochila" then 
									tipo_rota = "mochila"
									locs = mochila
								end
								...
								if tipo_rota ~= "" then 
									...
									TriggerEvent("Notify","sucesso","Você entrou em serviço da rota.",1500)
								...
								if not emP.checkPermission(value.permissao) then
									TriggerEvent("Notify","importante","Não tem permissão.",5000)
								else
									SendNUIMessage({ iniciarRota = true })
									SetNuiFocus(true, true)
									-- servico = true
									-- selecionado = 1
									-- CriandoBlip(locs,selecionado)
									-- TriggerEvent("Notify","sucesso","Você entrou em serviço.",1500)
									-- TriggerEvent("Notify","importante","Vá até o próximo local e colete as encomendas.",5000)
								end
							end
						end
				...
				if distance <= 2.0 then
					...
					if IsControlJustPressed(0,38) and time_co < 0 then
						-- if IsVehicleModel(vehicle,GetHashKey("nemesis")) and not IsPedInAnyVehicle(ped) then
							...
							local pags = pagamento[tipo_rota]

							for k, value in pairs(pags) do 
								local qtd_r =  math.random(value.qtd_min, value.qtd_max)
								if qtd_r > 0 then 
									emP.checkPayment(value.item, qtd_r)
								end
							end
...
RegisterNUICallback("setarRota",function(data)  
	tipo_rota = data.tipo_rota
end)

RegisterNUICallback("iniciarRotaSul",function(data)

	-- if not emP.checkPermission("rota") then
	-- 	TriggerEvent("Notify","importante","Não tem permissão.",5000)
	-- else
		...
		if tipo_rota == "" then
			...
			if tipo_rota == "arma" then 
				if emP.checkItem("bluecard",1) then 
					servico = true
				end
			end

			if tipo_rota == "lavagem" then 
				if emP.checkItem("redcard",1) then 
					servico = true
				end
			end

			if tipo_rota == "eletronic" or tipo_rota == "plastic" then 
				servico = true
			end
