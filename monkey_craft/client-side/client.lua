...
local crafts
-- local lista_craft = {nomeItem = value.nome_item, qtde = value.quantidade, tempo = value.tempo}
...
RegisterNUICallback('craftRecipe', function(data, cb)
    ...
    while lista_craft do
        lista_craft.tempo = lista_craft.tempo - 1
        SendNUIMessage({tempo = lista_craft.tempo})
        Citizen.Wait(time)
    end
